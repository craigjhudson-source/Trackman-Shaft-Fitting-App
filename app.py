...
def get_data_from_gsheet():
    try:
        ...
        data = {tab: pd.DataFrame(sh.worksheet(tab).get_all_records()) for tab in ['Heads', 'Shafts', 'Questions', 'Responses', 'Config']}
        return data
...
if all_data:
    if not st.session_state.interview_complete:
        ...
        # Use a Form to force-capture all fields including Auto-fills
        with st.form(key=f"form_{st.session_state.form_step}"):
            ...
            for _, row in q_df.iterrows():
                ...
                else: # Text Inputs (Fix for Autofill)
            ...
            if st.session_state.form_step < len(categories) - 1:
                submit = st.form_submit_button("Next ➡️")
                if submit:
                    st.session_state.form_step += 1
                    st.rerun()
            else:
                generate = st.form_submit_button("🔥 Generate Prescription")
                if generate:
        ...
        # Back button outside form (so it doesn't trigger validation)
