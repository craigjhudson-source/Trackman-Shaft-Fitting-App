...
with st.sidebar:
    ...
    feel_label = st.select_slider("Feel Priority:", options=["1", "2", "3", "4", "5"], value="3")
    
    st.header("🎯 Manual Targets")
    t_flex = st.number_input("Target FlexScore", value=6.0, step=0.1)
    t_launch = st.number_input("Target LaunchScore", value=5.0, step=0.5)
    tempo = st.selectbox("Tempo", ["Smooth", "Moderate", "Aggressive"])

# --- 3. MAIN LOGIC (Called Last) ---
...
if all_data:
    ...
    with col1:
        ...
        if tm_file:
            ...
            if stats:
                st.metric("Avg Speed", f"{stats['speed']:.1f} mph")
                st.write(f"Recommended Flex: {round((stats['speed']/10)-4, 1)}")
